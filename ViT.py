...
class ViT(nn.Module):
	def __init__(self, chw, n_patches=7, n_blocks=2, hidden_d=8, n_heads=2, out_d=10):
		...
		self.linear_mapper = nn.Linear(self.input_d, self.hidden_d)	#Linear(in_features_length, out_features_length, [...])
		self.linear_mapper_mask = nn.Linear(self.input_d, self.hidden_d)	

		# 2) Learnable classification token
		self.class_token = nn.Parameter(torch.rand(1, self.hidden_d))	#Generate one extra token for each of the enbeddings that comes out the Linear Mapper
